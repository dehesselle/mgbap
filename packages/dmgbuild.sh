...
DMGBUILD_CONFIG="$SRC_DIR"/gimp_dmg.py
...
function dmgbuild_install
{
  ...
  #       dmgbuild dependencies need Python 3.8,
  #       current workaround is to use 3.8 for JHBuild and use that here
  jhbuild run $JHBUILD_PYTHON_PIP install --prefix "$VER_DIR" $DMGBUILD_PIP
...
function dmgbuild_run
{
  ...
  # set application
  sed -i '' "s/PLACEHOLDERAPPLICATION/$(sed_escape_str "$GIMP_APP_DIR")/" "$DMGBUILD_CONFIG"
  ...
  dmgbuild -s "$DMGBUILD_CONFIG" "$(basename -s .app "$GIMP_APP_DIR")" "$TMP_DIR"/"$(basename "$dmg_file")"
